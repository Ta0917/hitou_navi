...
## 検索結果TOP3カード・Detailsセクションの夜の湯宿デザイン実装メモ（2026-07-18）

ユーザーから渡された `design_handoff_yushuku`（Claude Design、検索結果〜詳細ページの
「夜の湯宿」テーマ見本、ZIP配布）に基づき、トップページの検索結果エリアを再構築した。

### スコープの確認

見本README上は「検索結果〜詳細を独立した1ページ」として設計されていたが、実仕様は
「検索してもページ遷移せず、TopPage内にTOP3が表示される」現行方式が正。詳細ページは
別ルート（`/onsens/:slug`）のまま。よって見本のうち **Candidatesセクション（TOP3カード）と
Detailsセクション（4.3〜4.5節）** を、既存TopPage内の検索結果エリアに統合する形で実装した。

### 配色マッピング方針

見本CSS変数の生値をそのまま使わず、**湯あかり公式パレット**（`project_colorpalette`、
23色）にマッピングする。例：`--paper`→枯草色`#DCCDA3`、`--paper-mute`→海松茶`#7E765F`、
`--accent`→既存の`ACCENT_OVERNIGHT`/`ACCENT_DAYTRIP`（朱`#A8412F`/苔緑`#6F7E4F`、tripTypeで
切替、そのまま流用）、アクセス難易度4以上の警告色は`WARN_COLOR = '#a8412f'`（朱）に固定。

### タグ表示のためのバックエンド拡張

検索結果カードにタグを出すため、`OnsenSummaryResponse`に`tags: List[str] = []`を追加。
`models.py`の`Onsen`クラスに`tags`プロパティ（`onsen_tags`のうち`status='approved'`を
confidence降順で最大3件、`tag.label`のリストを返す）を新設。カラム追加ではないため
DB再シード不要。`main.py`の`GET /onsens`クエリにも`joinedload(Onsen.onsen_tags).joinedload(OnsenTag.tag)`
を追加（N+1回避）。`POST /search`側は`search.py`の`hard_filter`が元々同じeager loadを
行っていたため変更不要だった。

### 検索結果カード（`ResultCard`）

- サムネイル4:3、タグ行（最大3件）、スコアはドットバー方式（`ScoreDotBars`、5セグメント、
  ラベルは正式名称「静けさ／ソロ適性／アクセス難易度」を使うこと。短縮形「ひとり／たどり」は
  ユーザーフィードバックで不採用になった経緯がある）。
- ランクバッジ（「第◯席」表記）は一度実装したがユーザーフィードバックで撤去済み。
  `kanjiNum`ヘルパー・`.cand-rank-badge`CSSも合わせて削除した。もし再実装する場合は
  `KANJI_RANK`（Detailsセクション用に残っている3件版の配列）を参考にできる。

### TOP3以降を含む横スクロールカード列

検索結果は「TOP3を含む全件を1本の横スクロールカード列」として表示する（TOP3だけ別枠・
それ以降は縦リスト、という初期実装はユーザーフィードバックで撤回した）。

**重要な実装上の制約**：`scrollLeft`への直接代入や`el.scrollTo()`による連続アニメーションは
特定のChrome+GPU構成で描画更新されないバグがある（[[feedback-scroll-animation-transform]]
参照、AGENTS.mdにも同内容記載）。そのため横スクロールは既存のサジェストチップスクロール
（`scrollCallback`）と同じ「外側div: overflow:hidden + 内側div: transform」パターンを
`resultsScrollCallback`として新設し踏襲した。サジェストと異なり無限ループはせず、
`clamp()`で両端に達したら止まる有限スクロール。新しい検索のたびに`searchSeq` state
（検索成功ごとにインクリメント）を横スクロール外側divの`key`にして強制再マウントし、
スクロール位置を先頭に戻している（同じDOMノードが使い回されるとposXクロージャが前回検索の
値のまま残るため）。

### Detailsセクション（TOP3の詳細、スクロール連動フル実装）

見本4.3〜4.5節をスクロール連動込みでフル実装した：
- `StageImage`：sticky（モバイルはfixed）配置のステージ内に3枚重ね、`is-active`クラスで
  クロスフェード（`.stage-img-yushuku`、opacity/transform transition、index.css）。
  背景は各施設の`hero_image_url`（実写真）を使用。
- `DetailBlock`：地域/店名/リード文（`intro_text`の一文字目を字下げ風に強調）/
  雰囲気・泉質・行き方・混みあいの2列グリッド/大型3軸スコア(`BigScoreBars`)/
  「湯宿を見る→」「栞を挟む」ボタン。
- `SectionIndicator`：画面右固定の「now viewing」（モバイルは`writing-mode: vertical-rl`）。
- `CompareCta`：スクロールでフェードイン、ワイプホバー（`.compare-btn-yushuku`、CSS変数
  `--compare-accent`でtripType連動）。

**スクロール連動ロジック**（見本`refreshIndicator()`/`refreshCompareCta()`相当）：
`candidatesRegionRef`（横スクロール行）・`detailRefs`（3つのdetailブロック）・
`compareCtaRef`をrefで保持し、`useEffect(() => {...}, [results])`で`scroll`/`resize`
リスナー登録。ビューポート35%地点に最も近いdetailブロックを`activeDetailIdx`として
active判定し、ステージクロスフェード・インジケータを同期。TOP3行が画面上端より上に
出たら（`candidatesRegionRef`の`getBoundingClientRect().bottom < 80`）インジケータ表示を
許可するゲート条件も見本どおり。

**データが無い項目の扱い（捏造データは使わない方針）**：見本モックには「雰囲気」
「混みあい」が直接フィールドとして存在するが、実DBには対応カラムが無い。
- 雰囲気 → `onsen.tags[0]`（承認済みタグの最上位、実データ）で代用
- 混みあい → `quietness_comment`（DB既存カラム。静けさスコアのルーブリック自体が
  元々混雑傾向の説明を含むため意味的に流用可能と判断）で代用
- 「栞を挟む」→ お気に入り機能自体が未実装のため、ローカル表示専用トグル
  （`bookmarks` state、保存・同期なし）
- 「比較する」→ 比較ページ自体が未実装のため、見本と同じデモ動作（クリックで
  「比較ページへ…」に1.4秒だけ切り替わる、実遷移なし）。見本README自体が
  「本番は実際の遷移処理に置き換える」と明記した仮ボタンのため見本どおりの再現とした。
- 「Match%」バッジ（TOP3カードの適合度表示）は検索APIに実データが無いため実装しなかった
  （捏造値は出さない）。

### 検証環境メモ：Browser paneのcomputer/computed styleが不安定なことがある

このセッションで、Browser paneの`screenshot`/`zoom`が常にタイムアウトし、また
`getComputedStyle()`がinline styleと食い違う・Reactのクリックイベント後のstate反映が
`javascript_tool`の再読み取りで確認できない、という事象が発生した（[[project-browser-pane-instability]]
参照）。素のDOM `addEventListener`ではイベント発火は確認できており、コンソールに
`[vite] hot updated`が多数・`Failed to reload`エラーも出ていたため、agent-syncが前提とする
「別セッション（Codex等）による同一ファイルの同時編集起因のHMR干渉」が原因の可能性が高い。
このプロジェクトのブラウザ検証では、`getComputedStyle`より`element.style`（inline style、
DOM上の生の事実）を優先して読むこと。
